                        ...
                        <div class="row align-items-center mb-2">
                            ...
                            <div class="col-auto text-end float-end ms-auto download-grp">
                                @if (Auth::user()->hasPermissionTo('Create_Order'))
                                    <a href="{{ route('business.order.create.form') }}" class="btn btn-primary ms-2">
                                        +&nbsp;New Order
                                    </a>
                                @endif
                            </div>
    ...
    <script>
        ...
        function orderStatus(id, status) {
            $.confirm({
                ...
                icon: 'far fa-question-circle text-danger',
                title: 'Are you Sure!',
                content: 'Do you want to change the state of Selected Order?',
                type: 'red',
                autoClose: 'cancel|10000',
                buttons: {
                    confirm: {
                        ...
                        action: function() {
                            ...
                            $.ajax({
                                type: "POST",
                                url: "{{ route('business.order.change') }}",
                                data: data,
                                success: function(response) {
                                    if(response.status == 'error'){
                                        errorPopup(response.message, '')
